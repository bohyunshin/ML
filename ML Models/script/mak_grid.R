...
grid_bando = make_grid(126.1, 34.2, 129.5, 38.2)
...
make_leaflet_species = function(df, leaflet_, bird_name){
  ...
  result = leaflet_ %>% addCircles(~lon, ~lat, data=sel_data, color='black')
  return(result)
}

ex = make_leaflet_species(bird_jeju, leafleft_observed_grid_jeju, '뻐꾸기')
ex = make_leaflet_species(bird_bando, leafleft_observed_grid_bando, '뻐꾸기')
ex

make_grids_in_one_grid = function(grid, index){
  ...
  leaflet_ = leaflet() %>%
    setView(lng=(min_lon+max_lon)/2, lat=(min_lat+max_lat)/2, zoom=15) %>%
    addTiles()
  
  coords_in_one_grid = data.frame(coords_in_one_grid)
  colnames(coords_in_one_grid) = c('lon','lat')
  
  result = leaflet_ %>% addCircles(~lon, ~lat, data=coords_in_one_grid, color='green') %>%
              addRectangles(lng1=min_lon, lat1=min_lat,
...
make_grids_in_one_grid(grid_bando,44000)
...
leaflet_ = leaflet() %>%
  ...
  addRectangles(lng1=126.1, lat1=34.2,
                ...
                fillOpacity=0.2, opacity = 0.2)
leaflet_
# make bird, bnin, insect, flr observed circles
leaflet_ = leaflet() %>%
  setView(lng=128, lat=35, zoom=8) %>%
  addTiles() %>%
  addCircles(~coords.x1, ~coords.x2, data=data.frame(bird_coord), color='green')
leaflet_
head(bnin_coord)
flr@coords
...
################ for bando ################
r <- getData("worldclim", var="bio", res=0.5, lon=c(126, 129.5), lat=c(34, 38))
# lets also get the elevational data associated with the climate data
alt <- getData("worldclim", var="alt", res=.5, lon=c(126, 129.5), lat=c(34, 38))
...
save.image('/Users/shinbo/Desktop/contest/environ/workspace/workspace3.RData')
...
X_jeju = c()
a = 1
for (index in unique(index_grid_jeju)){
  temp = cal_stat_in_one_grid(grid_jeju, r, alt, index)
  X_jeju = rbind(X_jeju,temp)
  print(sprintf('%s finished out of 138', a))
  a = a + 1
}
X_jeju = data.frame(X_jeju)
X_jeju$grid_index = unique(index_grid_jeju)
X_jeju$region = 'j'
rownames(X_jeju) = NULL

# concat bando, jeju dataset
X_final = rbind(X_bando, X_jeju)

make_data_species = function(spc, bird_data, whole_observed_grid, left_merge_mat){
  spc_data = bird_data %>% filter(spcs_korea == spc)
  tmp1 = data.frame(grid_index = unique(spc_data$index_grid), y= rep(1, length(unique(spc_data$index_grid))))
  unobserved_grid = setdiff(whole_observed_grid, unique(spc_data$index_grid))
  tmp2 = data.frame(grid_index = unobserved_grid, y= rep(0,length(unobserved_grid)))
  y = data.frame(rbind(tmp1,tmp2))
  concat_mat = merge(left_merge_mat,y,id = 'grid_index')
  return(concat_mat)
}

# write data to csv
setwd('/Users/shinbo/Desktop/contest/environ/script')
make_spcs_data = function(bird_bando, bird_jeju, X_jeju, X_bando, spcs_name){
  kka_jeju = make_data_species(spcs_name, bird_jeju, unique(bird_jeju$index_grid), X_jeju)
  kka_bando = make_data_species(spcs_name, bird_bando, unique(bird_bando$index_grid), X_bando)
  kka_concat = rbind(kka_jeju, kka_bando)
  write.csv(kka_concat, paste('../concat_data/',spcs_name,'.csv', sep=''))
}

for (bird_name in unique(bird$spcs_korea)){
  make_spcs_data(bird_bando, bird_jeju, X_jeju, X_bando, bird_name)
}

train_test_split = function(data, ratio, seed_num){
  n = dim(data)[1]
  ...
  result = list()
  result[[1]] = data[train_index,]
  result[[2]] = data[test_index,]
  return(result)
}
split_data = train_test_split(kka_concat,0.7,1)
train = split_data[[1]]
test = split_data[[2]]

colnames(train)
formula = y ~ lon+lat+bio1_110+bio12_110
formula_all = y~.-grid_index-region
logistic_mod = glm(formula_all, data=train, family=binomial(link = "logit"))
summary(logistic_mod)
predict(logistic_mod, test[,c('lon','lat','bio1_110','bio12_110')])
